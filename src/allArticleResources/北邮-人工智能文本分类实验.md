...
我说：“期末考试没打印课件。”老师听了可高兴了，奖了我最喜欢的61分。
...
- 选择分类算法（朴素贝叶斯（必做）、SVM/其他~~**实际验收有几个不做的？**~~等），训练文本分类器，理解所选的分类算法的建模原理、实现过程和相关参数的含义；
- 对测试集的文本进行分类
...
### 数据集准备
[THUCNews](http://thuctc.thunlp.org/message)

数据集大小：1.45GB

样本数量：80多万

数据集详情链接：[thuctc](http://thuctc.thunlp.org)
...
![](./北邮-人工智能文本分类实验/thu.png)
<img src="北邮-人工智能文本分类实验/thu.png" style="width=75%">

### cppjieba
[cppjieba](https://github.com/yanyiwu/cppjieba)是"结巴(Jieba)"中文分词的C++版本。
...
$<a_1,a_2,\dots ,a_n>$下，得到最可能的目标值$V_{MAP}$。


$$
    \begin{aligned}
        v_{MAP} & ={arg\max}_{v_j}P(v_j|a_1,a_2,\dots ,a_n)                                \\      
                & ={arg\max}_{v_j\in V}\frac{P(a_1,a_2,\dots ,a_n)P(v_j)}{P(a_1,a_2,\dots ,a_n)} \\
                & ={arg\max}_{v_j\in V}P(a_1,a_2,\dots,a_n)P(v_j)                         \\       
                & ={arg\max}_{v_j\in V}P(v_j)\prod_i P(a_i|v_j)
    \end{aligned}
$$

采纳m-估计方法，即有统一的先验概率并且m等于词汇表的
大小，因此
$$
    \begin{aligned}
        P(w_k|v_j)=\frac{v_k+1}{n+|Vocabulary|}
    \end{aligned}
$$
...
$$
\begin{aligned}
    \chi^2(t,c)=\frac{N(AD-BC)^2}{(A+B)(A+C)(C+D)(B+D)}
\end{aligned}
$$

### svm
下载地址：[libsvm](http://www.csie.ntu.edu.tw/~cjlin/cgi-bin/libsvm.cgi?+http://www.csie.ntu.edu.tw/~cjlin/libsvm+tar.gz)
...
void *svmData(void *t) {
    int type = (long long)t;
    //先打开训练数据文件
    int fdout = open((svmTrainData + V[type]).c_str(), O_WRONLY | O_CREAT);
    if (fdout == -1) {
        cout << errno << '\n';
        perror("open");
    }
    ...
    for (int d = 0; d < (int)fileList[type].size() && d < 2 * docs[type]; ++d) {
        if (d == docs[type]) {  //这之后是测试数据集
            close(fdout);
            fdout = open((svmTestData + V[type]).c_str(), O_WRONLY | O_CREAT);
            fdoffset = 0;
            if (fdout == -1) {
                cout << errno << '\n';
                perror("open");
...
void makeSvm() {
    ...
    char buf[MAXS];
    int fdout =
        open((svmTrainData + "trainData.txt").c_str(), O_WRONLY | O_CREAT);
    if (fdout == -1) {
        cout << errno << '\n';
        perror("open");
    }
    ...
    fdout = open((svmTestData + "testData.txt").c_str(), O_WRONLY | O_CREAT);
    off = 0;
    for (int i = 0; i < Vtot; ++i) {
...
![结果](北邮-人工智能文本分类实验/ans1.png)

预测完成之后是混淆矩阵。

![](北邮-人工智能文本分类实验/ans2.png)

输出召回率和准确率，最后输出运行时间。
**可以看到最后运行时间仅为12秒，这就是c++的长处之一。**
总共分析了55000篇文章，从读入到分词，到其他计算。总共执行时间非常短。
相比于python的10min左右要好得多。

![结果](北邮-人工智能文本分类实验/ans3.png)
...
- 数据预测
 `./svm-predict test.scale my.model ans.out`

结果如下

![svman](./北邮-人工智能文本分类实验/svmans.png)
...
[超链接](https://paste.ubuntu.com/p/p9KQmh8Q2T/)
