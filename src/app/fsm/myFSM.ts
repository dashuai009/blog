var greekLetterNames: string[] = ['Alpha', 'Beta', 'Gamma', 'Delta', 'Epsilon', 'Zeta', 'Eta', 'Theta', 'Iota', 'Kappa', 'Lambda', 'Mu', 'Nu', 'Xi', 'Omicron', 'Pi', 'Rho', 'Sigma', 'Tau', 'Upsilon', 'Phi', 'Chi', 'Psi', 'Omega'];

var canvas;
var nodeRadius: number = 30;

var nodes: MyNode[] = [];
var links: (Link | SelfLink | StartLink)[] = [];
var cursorVisible = true;
var snapToPadding = 6; // pixels eps 可容忍的误差值
var hitTargetPadding = 6; // pixels
//var selectedObject: Link | MyNode = null; // either a Link or a Node
var selectedObject: any = null;
var currentLink: Link | SelfLink | StartLink | TemporaryLink = null; // a Link
var movingObject = false;
var originalClick;//鼠标单击事件
...
class MyNode {//点
    ...
    mouseOffsetX: number;
    mouseOffsetY: number;
    isAcceptState: boolean;//是否是接受状态，两个圈    
    text: string;
    constructor(x: number, y: number) {//根据坐标构造
        this.x = x;
        this.y = y;
        this.mouseOffsetX = 0;
        this.mouseOffsetY = 0;
    ...
    closestPointOnCircle(x: number, y: number) {//把圆内或圆外的某个点伸张到圆上
        let dx = x - this.x;
        let dy = y - this.y;
        let scale = Math.sqrt(dx * dx + dy * dy);
        return new MyNode(this.x + dx * nodeRadius / scale, this.y + dy * nodeRadius / scale);
    };

    containsPoint(x: number, y: number): boolean {//该点所代表的圆（状态）是否包含点（x，y）
...
class StartLink {
    node: MyNode;//终点，箭头那一端
    deltaX: number;
    deltaY: number;//startLink的坐标变化值，起点-终点（有点反直觉
    text: string;
    //

    constructor(node: MyNode, start?: MyNode) {
        this.node = node;
        this.deltaX = 0;
        this.deltaY = 0;
        this.text = '';
    ...
    setAnchorPoint(x: number, y: number): void {//如果坐标变化值太小，就处理为无变化
    ...
    draw(c) {
        ...
        let textAngle = Math.atan2(stuff.startY - stuff.endY, stuff.startX - stuff.endX);
        drawText(c, this.text, stuff.startX, stuff.startY, textAngle, selectedObject == this);

        // draw the head of the arrow（箭头）
        drawArrow(c, stuff.endX, stuff.endY, Math.atan2(-this.deltaY, -this.deltaX));
    };

    containsPoint(x: number, y: number) {//该有向线段是否包含（x，y）这个点
        let stuff = this.getEndPoints();
...
class Link {
    ...
    getEndPointsAndCircle() {
        ...
        let circle = circleFromThreePoints(this.nodeA.x, this.nodeA.y, this.nodeB.x, this.nodeB.y, anchor.x, anchor.y);
        let isReversed = (this.perpendicularPart > 0);
        let reverseScale = isReversed ? 1 : -1;
        let startAngle = Math.atan2(this.nodeA.y - circle.y, this.nodeA.x - circle.x) - reverseScale * nodeRadius / circle.radius;
        let endAngle = Math.atan2(this.nodeB.y - circle.y, this.nodeB.x - circle.x) + reverseScale * nodeRadius / circle.radius;
    ...
    draw(c) {
        ...
        if (stuff.hasCircle) {
            ...
            let textAngle = (startAngle + endAngle) / 2 + (stuff.isReversed ? Math.PI : 0);
...
class SelfLink {
    ...
    constructor(node: MyNode, mouse?: any) {
        this.node = node;
        this.anchorAngle = 0;
    ...
    getEndPointsAndCircle() {
        let circleX = this.node.x + 1.5 * nodeRadius * Math.cos(this.anchorAngle);
        let circleY = this.node.y + 1.5 * nodeRadius * Math.sin(this.anchorAngle);
        let circleRadius = 0.75 * nodeRadius;
...
class TemporaryLink {
    ...
    from: MyNode;
    to: MyNode;
    constructor(from: MyNode, to: MyNode) {
        this.from = from;
        this.to = to;
    }
...
function restoreBackup() {
    ...
    try {
        let backup = JSON.parse(localStorage['fsm']);

        for (let i = 0; i < backup.nodes.length; i++) {
            let backupNode = backup.nodes[i];
            let node = new MyNode(backupNode.x, backupNode.y);
            node.isAcceptState = backupNode.isAcceptState;
            node.text = backupNode.text;
            nodes.push(node);
        }
        for (let i = 0; i < backup.links.length; i++) {
            let backupLink = backup.links[i];
            let link = null;
...
function circleFromThreePoints(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number) {
    let a = det(x1, y1, 1, x2, y2, 1, x3, y3, 1);
    let bx = -det(x1 * x1 + y1 * y1, y1, 1, x2 * x2 + y2 * y2, y2, 1, x3 * x3 + y3 * y3, y3, 1);
    let by = det(x1 * x1 + y1 * y1, x1, 1, x2 * x2 + y2 * y2, x2, 1, x3 * x3 + y3 * y3, x3, 1);
    let c = -det(x1 * x1 + y1 * y1, x1, y1, x2 * x2 + y2 * y2, x2, y2, x3 * x3 + y3 * y3, x3, y3);
    return {
        'x': -bx / (2 * a),
        'y': -by / (2 * a),
...
function fixed(number: number, digits: number) {
...
function textToXML(text: string) {
...
function drawText(c, originalText, x: number, y: number, angleOrNull, isSelected) {
...
export function onLoad() {
    ...
    canvas.onmousedown = function (e) {
        ...
        if (selectedObject != null) {
            if (shift && selectedObject instanceof MyNode) {
    ...
    canvas.ondblclick = function (e) {
        ...
        if (selectedObject == null) {
            selectedObject = new MyNode(mouse.x, mouse.y);
            nodes.push(selectedObject);
            resetCaret();
            draw();
        } else if (selectedObject instanceof MyNode) {
    ...
    canvas.onmousemove = function (e) {
        ...
        if (currentLink != null) {
            ...
            if (selectedObject == null) {
                if (targetNode != null) {
                    currentLink = new StartLink(targetNode as MyNode, originalClick);
                } else {
                ...
                if (targetNode == selectedObject) {
                    ...
                    currentLink = new Link(selectedObject, targetNode as MyNode);
                } else {
        ...
        if (movingObject) {
            ...
            if (selectedObject instanceof MyNode) {
    ...
    canvas.onmouseup = function (e) {
        ...
        if (currentLink != null) {
            if (!(currentLink instanceof TemporaryLink)) {
                ...
                links.push(currentLink as Link | SelfLink | StartLink);
                resetCaret();
            }
            currentLink = null;
            draw();
...
document.onkeydown = function (e) {
    ...
    if (key == 16) {
        ...
        if (selectedObject != null) {
            ...
            for (let i = 0; i < links.length; i++) {
                if (links[i] == selectedObject || (links[i] as StartLink | SelfLink).node == selectedObject || (links[i] as Link).nodeA == selectedObject || (links[i] as Link).nodeB == selectedObject) {
                    links.splice(i--, 1);
                }
            }
            selectedObject = null;
...
function crossBrowserRelativeMousePos(e): MyNode {
    let element = crossBrowserElementPos(e);
    let mouse = crossBrowserMousePos(e);
    return new MyNode(mouse.x - element.x,
        mouse.y - element.y);
    ...
    this.advancedFillText = function (text, originalText, x, y, angleOrNull) {
        if (text.replace(' ', '').length > 0) {
            ...
            if (angleOrNull != null) {
                ...
                let dx = Math.cos(angleOrNull);
                let dy = Math.sin(angleOrNull);
